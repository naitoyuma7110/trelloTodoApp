...
const Sample = () => {
  ...
  const dragOverHandler = (event: DragOverEvent) => {
    ...
    if (active.id !== over.id) {
      ...
      const updatedTodos = items.map((item) => {
        ...
        return item.id === String(active.id) ? { ...item, status: overColumn || overId } : item
      })
  ...
  return (
    <div className='grid grid-cols-3'>
      <DndContext sensors={sensors} collisionDetection={closestCorners} onDragOver={dragOverHandler}>
        {statuses.map((status: Status) => {
          const filteredTodo = items.filter((item) => {
            return item.status === status
          })
          return <SampleColmun key={status} todos={filteredTodo} status={status} />
        })}
      </DndContext>
    </div>
  )
